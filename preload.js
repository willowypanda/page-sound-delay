(() => {
  ...
  const css = `
    #psd-panel { position: fixed; z-index: 2147483647; right: 18px; bottom: 18px; width: 360px; color: #e8edf2; background: rgba(22,27,34,.96); border: 1px solid #48515b; border-radius: 12px; box-shadow: 0 8px 30px rgba(0,0,0,.45); font: 14px/1.4 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; padding: 14px; }
    #psd-panel * { box-sizing: border-box; }
    #psd-panel .psd-title { display:flex; justify-content:space-between; align-items:center; font-weight:700; margin-bottom:10px; }
    #psd-panel .psd-row { display:flex; gap:6px; align-items:center; margin:7px 0; flex-wrap:wrap; }
    #psd-panel button, #psd-panel input { border:1px solid #59636d; border-radius:6px; color:#edf2f7; background:#28313a; padding:6px 8px; }
    #psd-panel button { cursor:pointer; }
    ...
    #psd-panel input[type=number] { width:110px; }
    #psd-panel input[type=text] { min-width:0; flex:1; }
    #psd-panel .psd-value { font-size:18px; font-weight:700; min-width:52px; text-align:center; }
    #psd-panel .psd-status { color:#9fb1c2; font-size:12px; min-height:18px; }
    #psd-panel .psd-help { color:#91a0ae; font-size:11px; margin-top:8px; }
  ...
  function makePanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="psd-row"><input id="psd-manual" type="number" min="0" max="120" step="0.1" placeholder="延时秒数"><button id="psd-apply">应用延时</button><button id="psd-measure">测量延时</button><span id="psd-timer">0.0s</span></div>
      <div class="psd-row"><label><input id="psd-enabled" type="checkbox"> 启用声音延时</label><button id="psd-resume">播放 / 恢复音频</button></div>
